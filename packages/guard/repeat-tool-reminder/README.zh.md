...
本包帮助模型跳出以相同参数反复调用同一工具却没有进展的循环。达到配置的重复次数时，它会要求模型检查上一次结果并改变方法或结束任务。提醒只是建议，绝不会阻止或延迟合理的重复调用。每个 agent 的重复分别跟踪，新的用户消息会清除计数。`dsh` base 组合默认启用本包，并在重复 3、5、8 次时提醒。可选的 `hardStop` 配置会添加第二种行为——一个派发前的熔断器，在工具主体运行之前拒绝连续相同次数达到所配置长度的受跟踪调用。决策记录见 [repeat-tool-guard Agent Note](../../../.agents/notes/archived/feature/2026-07-08-repeat-tool-guard.md)；硬熔断记录见 [硬熔断 Agent Note](../../../.agents/notes/implemented/bug-fix/2026-08-26-repeat-tool-hard-stop.zh.md)。
...
当模型长时间自主工作、且卡住的循环是你想用建议而非强制来打破的失败模式时，选择它。当相同的重复是合理且必须不受打扰地运行时——guard 只会提醒，提醒只是重复调用之后的一条小消息——以及必须捕获近似变体时（因为只有精确重复——同一工具、同一参数且与属性顺序无关——才会被检测到），避免使用它。当循环必须停止执行而不是继续收取提醒时——例如本来会白白消耗多轮或 token 的工具发射循环——选择 `hardStop` 选项。
...
| `hardStop.enabled` | `false` | 为 `true` 时，在派发前拒绝连续次数达到 `hardStop.at` 的受跟踪调用 |
| `hardStop.at` | `10` | 触发熔断器的连续相同受跟踪调用次数 |

无效配置会在启动时以清晰错误失败——空的 `thresholds` 列表、小于 2 的重复次数、重复值或小于 2 的 `hardStop.at`——绝不会静默改变行为。生成的[配置目录](../../../docs/config-catalog.zh.md#deepseek-aidsh-repeat-tool-reminder)记录每个受支持的值。
...
启用 `hardStop.enabled: true` 时，连续相同次数将达到 `at` 的受跟踪调用会在派发前被拒绝——工具主体不会运行——之后的相同尝试持续被拒绝，直到 agent 更换工具或参数；拒绝原因会作为被拒绝调用的错误结果送达模型。请把 `at` 设在最高 `thresholds` 条目之上，让逐级提醒在熔断器跳闸前先升级。
...
- **默认仅建议，可选才否决。** guard 用模型上下文丰富 post-execute 决策；除非 `hardStop.enabled` 打开它自己的派发前拒绝，否则它从不阻止或改写调用。
- **在 post-execute 中计数。** 检测运行在 `tools/post-execute` 上，被拒绝的调用同样会经过它；在那里计数让一个监听器即可覆盖所有尝试——包括被硬熔断拒绝的尝试——无需跨事件状态。
...
- **加载时快速失败。** `thresholds`、`argumentsPreviewChars` 与 `hardStop.at` 在 `apply` 中校验并抛出错误，绝不回退到默认值。
...
- **硬熔断在派发前拒绝。** 启用 `hardStop.enabled` 后，guard 自己的 `tools/pre-execute` 监听器会拒绝连续次数达到 `at` 的受跟踪调用——工具主体不会运行，后续的 pre-execute 监听器也看不到该调用，拒绝原因会成为该调用的错误结果。被拒绝的调用仍会经过 post-execute，因此会计入链：相同尝试持续被拒绝，直到 agent 更换工具或参数，此时链照常重置。
...
| — | 不发布运行时不变式伴生入口；链私有于 guard 自己的监听器，不暴露独立伴生可观察的包级事件或快照。 |
...
熔断器跳闸时，被拒绝调用的错误结果携带下面的拒绝文本，因此它必须能独立充当纠正信号；与详细提醒相同的参数预览上限会约束它。
...
跳闸前 guard 产生零 token；每次被拒绝的尝试都以该调用的错误结果呈现拒绝文本，并受预览上限约束。
...
- **阻止需要 `hardStop` 选项**——仅提醒绝不阻止或延迟调用；派发前拒绝只存在于 `hardStop.enabled` 之下。
- **subagent 之间不共享链**——链始终按 agent 隔离；父 agent 与其 subagent 重复相同调用也绝不合并。
- **合理的幂等轮询超过阈值后仍会收到提醒**——可通过 `thresholds`／`exclude` 配置释放压力。
- **超过最高阈值后链不再提醒**——提醒只在精确达到所配置的次数时触发，超过后不会继续发送；已启用的硬熔断则会继续拒绝相同的尝试。
...
[repeat-tool-guard Agent Note](../../../.agents/notes/archived/feature/2026-07-08-repeat-tool-guard.md) 以旧包名记录了原始设计与备选方案；[改名台账](../../../.agents/notes/archived/architecture/2026-08-11-repository-naming-contract-and-rename-ledger.md) 记录了改名为 `repeat-tool-reminder` 及其原因。[硬熔断 Agent Note](../../../.agents/notes/implemented/bug-fix/2026-08-26-repeat-tool-hard-stop.zh.md) 记录了熔断器设计与促成它的事故。
